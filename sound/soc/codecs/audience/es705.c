 ...
 *
 *       Jeremy Pi<jpi@audience.com>
 *            - Separate sequence of keyword and LPSD for UART CVQ buffering @2014-07-18
 *            - Exception case from vs streaming status to normal @2014-07-21
...
#define FEATURE_CVQ_STREAM_UART_ONLY
#define FEATURE_CVQ_VS_ONLY
#define ES705_NO_EVENT			0x0000
#endif

#ifdef FEATURE_CVQ_VS_ONLY
...
#else
#define ES705_CVS_PRESET_CMD 0x906f1b59 /* 7001 */

#define ES705_INT_OSC_MEASURE_START	0x8070
#define ES705_INT_OSC_MEASURE_QUERY	0x8071
#endif
...
#define ES705_WAKE_LOCK_TIME (HZ * 1)	/* 1 sec */
#if defined(CONFIG_SND_SOC_ES705_FOR_VOIP)
#define NETWORK_OFFSET_VOIP	4
#endif
...
static void es705_restore_bwe_veq(int pre_bwe, int pre_veq);
...
static int cnt_restore_std_fw_in_vs = 0;
...
static void es705_switch_route_config(long route_index)
{
	...
	case 32 : /* 3Mic SPK, NB, NS On, CP2 */
	case 33 : /* 3Mic HS, NB, NS On, CP2 */
	case 34 : /* 3Mic SPK, NB, NS Off, CP2 */
	case 35 : /* 3Mic HS, NB, NS Off, CP2 */
#endif
...
#if defined(CONFIG_SND_SOC_ES705_FOR_VOIP)
		if ((route_config_idx == 32) || (route_config_idx == 33)) {
				route_config_idx += NETWORK_OFFSET_VOIP;
				dev_dbg(es705->dev,
					"%s() adjust wideband offset\n", __func__);
#if defined(PREVENT_CALL_MUTE_WHEN_SWITCH_NB_AND_WB)
				schedule_delayed_work(&es705->reroute_work,
...
#if defined(CONFIG_SND_SOC_ES705_FOR_VOIP)
		if ((route_config_idx == 36) || (route_config_idx == 37)) {
				route_config_idx -= NETWORK_OFFSET_VOIP;
				dev_dbg(es705->dev,
					"%s() adjust wideband offset\n", __func__);
#if defined(PREVENT_CALL_MUTE_WHEN_SWITCH_NB_AND_WB)
...
static void es705_switch_vs_streaming_to_normal_mode(struct es705_priv *es705)
{
	...
	rc = es705_write(NULL, ES705_POWER_STATE, ES705_SET_POWER_STATE_VS_OVERLAY);
	if (rc)
		dev_err(es705->dev, "%s(): Failed set power state to vs overlay\n", __func__);
	msleep(100);
...
static int es705_write_sensory_vs_keyword(void)
{
	...
	if (rc) {
		dev_err(es705_priv.dev, "%s(): request_firmware(%s) failed %d\n",
			__func__, es705_priv.grammar_path, rc);
		return rc;
	}
	...
	if (rc) {
		dev_err(es705_priv.dev, "%s(): request_firmware(%s) failed %d\n",
			__func__, es705_priv.net_path, rc);
		return rc;
	}
...
{
	struct es705_priv *es705 = &es705_priv;
	int pre_bwe = es705->current_bwe;
	int pre_veq = es705->current_veq;
	int rc;
	...
	if (value) {
		...
		es705_restore_bwe_veq(pre_bwe, pre_veq);
	}
}
#endif
				...
				const char *buf, size_t count)
{
	...
	index = strrchr(path, '/');
	if (index)
		strncpy(es705_priv.grammar_path, index + 1, strlen(index + 1));
	else {
		pr_info("%s : [ES705] cannot find /\n", __func__);
		strncpy(es705_priv.grammar_path, path, strlen(path));
	}
	pr_info("%s : [ES705] keyword_grammar_final_path = %s\n", __func__, es705_priv.grammar_path);
				...
				const char *buf, size_t count)
{
	...
	index = strrchr(path, '/');
	if (index)
		strncpy(es705_priv.net_path, index + 1, strlen(index + 1));
	else {
		pr_info("%s : [ES705] cannot find /\n", __func__);
		strncpy(es705_priv.net_path, path, strlen(path));
	}
	pr_info("%s : [ES705] keyword_net_final_path = %s\n", __func__, es705_priv.net_path);
...
{
	int rc = 0;
#ifdef SAMSUNG_ES70X_RESTORE_STD_FW
	u32 cmd = (ES705_SET_POWER_STATE << 16) |
			ES705_SET_POWER_STATE_VS_OVERLAY;
#endif
...
#endif /* SAMSUNG_ES70X_RESTORE_STD_FW */		
	mutex_unlock(&es705->pm_mutex);
	if (rc < 0) {
...
#if defined(SUPPORT_DAI_3)
	ports_active |= es705_priv.tx2_route_enable;
#endif
...
#if 0//defined(SUPPORT_DAI_3)
	ch_tot += es705_priv.dai[ES705_SLIM_2_CAP].ch_tot;
#endif
...
static int es705_wakeup(struct es705_priv *es705)
{
	...
	u32 sync_rspn = sync_cmd;
	int match = 1;
#ifndef SAMSUNG_ES70X_RESTORE_STD_FW
	int retry = 0;	
...
#else /* !SAMSUNG_ES70X_RESTORE_STD_FW */		
	/* retry wake up */
	retry = 0;
	do {
		rc = es705_write_then_read(es705, &sync_cmd, sizeof(sync_cmd),
...
#if defined(SUPPORT_DAI_3)
static int es705_get_tx2_route_enable_value(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = es705_priv.tx2_route_enable;
	dev_dbg(es705_priv.dev, "%s(): tx2_route_enable = %d\n",
		__func__, es705_priv.tx2_route_enable);

	return 0;
}

static int es705_put_tx2_route_enable_value(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	es705_priv.tx2_route_enable = ucontrol->value.integer.value[0];
	dev_dbg(es705_priv.dev, "%s(): tx2_route_enable = %d\n",
		__func__, es705_priv.tx2_route_enable);

#if defined(SAMSUNG_ES705_FEATURE)
	if (es705_priv.power_control) {
		if(es705_priv.tx2_route_enable)
			es705_priv.power_control(ES705_SET_POWER_STATE_NORMAL,
						 ES705_POWER_STATE);
		else if (!(es705_priv.rx1_route_enable ||
				es705_priv.rx2_route_enable ||
...
#ifndef SEAMLESS_VOICE_WAKEUP
static void es705_get_default_keyword(void)
{
	memset(es705_priv.grammar_path, 0, sizeof(es705_priv.grammar_path));
	sprintf(es705_priv.grammar_path, "higalaxy_en_us_gram6.bin");
	dev_info(es705_priv.dev, "%s(): default keyword grammar path %s\n", __func__,
		es705_priv.grammar_path);

	memset(es705_priv.net_path, 0, sizeof(es705_priv.net_path));
	sprintf(es705_priv.net_path, "higalaxy_en_us_am.bin");
	dev_info(es705_priv.dev, "%s(): default keyword net path %s\n", __func__,
		es705_priv.net_path);
}
#endif
		...
		if(es705_priv.voice_wakeup_enable == 1) { /* Voice wakeup */
...
#ifdef FEATURE_CVQ_STREAM_UART_ONLY
				es705_cmd(&es705_priv, ES705_CVS_PRESET_CMD);
				es705_priv.cvs_preset = (ES705_CVS_PRESET_CMD & 0xffff);

				dev_info(es705_priv.dev, "%s(): CVS preset = %d\n",
					__func__, es705_priv.cvs_preset);


				es705_cmd(&es705_priv, vs_keyword_length_cmd);
				dev_info(es705_priv.dev, "%s(): VS Keyword length = %d ms\n",
					__func__, es705_priv.vs_keyword_length);
				es705_priv.internal_route_num = 40; /* initialize route number */
#endif
			...
			if (es705_priv.es705_power_state == ES705_SET_POWER_STATE_VS_OVERLAY) {
				...
				if (es705_priv.es705_power_state == ES705_SET_POWER_STATE_NORMAL) {	
					value = es705_read(NULL, ES705_POWER_STATE);
					dev_info(es705_priv.dev, "%s(): current power state = %d\n",
						__func__, value);
				}
...
int es705_remote_route_enable(struct snd_soc_dai *dai)
{
	...
	case ES705_SLIM_2_CAP:
		return es705_priv.tx2_route_enable;
#endif
	default:
		return 0;
	}
...
int es705_put_veq_block(int volume)
{
	...
	if (network_type == WIDE_BAND) {
		if (extra_volume || es705->current_bwe)
			cmd = veq_noise_estimate_adjs_extra_wb[veq_use_case][volume];
		else
			cmd = veq_noise_estimate_adjs_wb[veq_use_case][volume];
	} else {
		if (extra_volume || es705->current_bwe)
			cmd = veq_noise_estimate_adjs_extra[veq_use_case][volume];
		else
...
static void es705_restore_bwe_veq(int pre_bwe, int pre_veq)
{
	...
	dev_info(es705->dev, 
			"%s(): pre_bwe : %d, current_bwe : %d\n",
			__func__, pre_bwe, es705->current_bwe);

	dev_info(es705->dev, 
			"%s(): pre_veq : %d, current_veq : %d\n",
			__func__, pre_veq, es705->current_veq);

	if (es705->internal_route_num != 5) {
		/* Restore BWE */
		if (pre_bwe != es705->current_bwe) {
		...
		/* Restore VEQ */
		if (pre_veq != es705->current_veq)
				es705_put_veq_block(pre_veq);
	}else
		dev_info(es705->dev, "%s(): Route %ld is not call state\n",
				__func__, es705->internal_route_num);
}
...
{
	struct es705_priv *es705 = &es705_priv;
	u32 es_set_power_level = ES705_SET_POWER_LEVEL << 16 | ES705_POWER_LEVEL_6;
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
...
#if defined(SUPPORT_DAI_3)
	SOC_SINGLE_EXT("ES705 TX2 Enable", SND_SOC_NOPM, 0, 1, 0,
			   es705_get_tx2_route_enable_value,
			   es705_put_tx2_route_enable_value),
#endif
	...
	SOC_SINGLE_EXT("Extra Volume Enable", SND_SOC_NOPM, 0, 1, 0,
	...
	SOC_SINGLE_EXT("Get Detected Keyword",
			...
			get_detected_keyword, NULL),
	SOC_SINGLE_EXT("VS Keyword Length",
			SND_SOC_NOPM, 0, 65535, 0,
			get_vs_keyword_length,
			put_vs_keyword_length),
#ifdef FEATURE_CVQ_STREAM_UART_ONLY
	SOC_ENUM_EXT("Switch Ext Osc", es705_vbb_enable_enum,
			   switch_get_osc, switch_ext_osc),
#endif
};
...
void check_detected_vs_keyword(struct es705_priv *es705)
{
	...
	rc = es705_write_then_read(es705, &chk_vs_keyword, sizeof(chk_vs_keyword),
								&rspn, match);
...
void check_vs_status(struct es705_priv *es705)
{
	...
	rc = es705_write_then_read(es705, &chk_vs_svscore, sizeof(chk_vs_svscore),
								&rspn, 0);
	if (rc < 0) {
	...
	if (rc < 0) {
		...
		vs_status_response = rspn & 0xFFFF;
		dev_info(es705->dev, "%s(): FinalScore is %d\n", __func__, vs_status_response);
	}
	rc = es705_write_then_read(es705, &chk_vs_noiseFloor, sizeof(chk_vs_noiseFloor),
								&rspn, 0);
	if (rc < 0) {
		dev_err(es705->dev, "%s(): cannot check the status\n", __func__);
	}
...
static void es705_event_status(struct es705_priv *es705)
{
	...
	u32 sync_cmd = (ES705_SYNC_CMD << 16) | ES705_SYNC_POLLING;
	u32 sync_ack;
	u32 int_osc_measure_start_cmd = (ES705_INT_OSC_MEASURE_START << 16) | 0x0000;
	u32 int_osc_measure_query_cmd = (ES705_INT_OSC_MEASURE_QUERY << 16) | 0x0000;
	u32 cmd_resp;
	...
	if (es705->voice_wakeup_enable == 1) {
		...
		if (es705->detected_vs_keyword == 1)
			es705->es705_power_state = ES705_SET_POWER_STATE_VS_STREAMING;
	}
#endif
	...
	if (es705->es705_power_state == ES705_SET_POWER_STATE_VS_LOWPWR) {
		...
		rc = es705_wakeup(es705);
		if (rc) {
			dev_err(es705->dev, "%s(): Get VS Status Fail - wakeup fail\n",
					__func__);
			return;
		} else {
			/* wait VS status readiness */
	...
	if ((event_response & 0x00FF) == KW_DETECTED) {
		if ((es705->voice_wakeup_enable == 1) &&
			...
			if (es705->uart_state != UART_OPEN) {
				...
				if (rc) {
					dev_err(es705->dev, "%s(): uart open failed %d\n",
					__func__, rc);
			...
			do {
				...
				rc = es705->cmd(es705, int_osc_measure_start_cmd, 0, &cmd_resp);
				if (rc)
					dev_err(es705->dev, "%s(): sending configure command on slim failed\n",
							__func__);

				rc = es705_uart_write(es705, &configure_uart,
							sizeof(configure_uart));
				if (rc < 0)
					dev_err(es705->dev, "%s(): sending configure command on uart failed\n",
							__func__);

				usleep_range(5000, 5005);
				rc = es705_uart_cmd(es705, sync_cmd, 0, &sync_ack);
				if (rc < 0)
					dev_err(es705->dev, "%s(): sending sync command failed - %d\n",
							__func__, rc);
				else
					dev_info(es705->dev, "%s(): sync ack is = 0x%08x\n",
							__func__, sync_ack);

				rc = es705->cmd(es705, int_osc_measure_query_cmd, 0, &cmd_resp);
				if (rc) {
					dev_err(es705->dev, "%s(): sending caliberation command on slim failed\n",
							__func__);
				} else if (cmd_resp == int_osc_measure_query_cmd) {
					dev_info(es705->dev, "%s(): caliberation was successful\n",
							__func__);
					break;
				} else {
					dev_info(es705->dev, "%s(): caliberation response 0x%08x\n",
							__func__, cmd_resp);
				}
			...
			if (!sync_retry) {
				dev_err(es705->dev, "%s(): UART internal clock is failed to configure, using fallback mechanism\n",
						__func__);

				rc = es705->cmd(es705, es_set_power_level, 0, &resp);
				if (rc < 0) {
					dev_err(es705->dev, "%s(): Error setting power level\n",
							__func__);
					goto voiceq_isr_exit;
				}
				usleep_range(2000, 2005);
...
*/
				...
				dev_info(es705->dev, "%s(): calibrating the uart was successful\n", __func__);
			}
...
voiceq_isr_exit:
			...
			if (es705_priv.cvs_preset != 0xFFFF &&
				...
				dev_info(es705->dev, 
					"%s(): Enable the ext clk and set power state to normal\n",
					__func__);
...
irqreturn_t es705_irq_event(int irq, void *irq_data)
{
	...
	if (es705_priv.voice_wakeup_enable == 0) {
		dev_info(es705->dev, "%s : voice wakeup disable setting, skip interrupt\n", __func__);
		mutex_unlock(&es705->cvq_mutex);
		return IRQ_HANDLED;
	}
#if defined(SAMSUNG_ES705_FEATURE)
	wake_lock_timeout(&es705_priv.voice_det_wakelock, ES705_WAKE_LOCK_TIME);
#endif
...
#if 0
#define SLIMBUS_PRESENT_TIMEOUT 5000
static int es705_slim_get_laddr(struct slim_device *sb,
...
int es705_core_probe(struct device *dev)
{
	...
	es705_priv.vs_keyword_length = 1000; /* default : 1000 ms (for "Hi Galaxy") */
	...
	es705_priv.keyword = device_create(es705_priv.svoice, NULL, 0, NULL, "keyword");
	...
	wake_lock_init(&es705_priv.voice_det_wakelock, WAKE_LOCK_SUSPEND, "es705_wakelock");
#endif
...
gpio_init_error:
#if defined(SAMSUNG_ES705_FEATURE)
	wake_lock_destroy(&es705_priv.voice_det_wakelock);
