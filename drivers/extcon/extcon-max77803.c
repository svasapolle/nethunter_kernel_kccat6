...
static const char const *max77803_path_name[] = {
	[PATH_OPEN]	= "OPEN",
	[PATH_USB_AP]	= "USB-AP",
	[PATH_AUDIO]	= "AUDIO",
...
static int max77803_muic_set_uart_sel_pass2
					  ...
					  const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "PDA", 3)) {
		...
		max77803_muic_set_usb_path(info, PATH_USB_CP);
	} else
					  ...
					  const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "AP", 2)) {
		int ret = max77803_muic_set_uart_sel_pass2
			(info, PATH_UART_AP);
		if (ret >= 0){
			info->muic_data->uart_sel = PATH_UART_AP;
...
#if 0
	ret = max77803_muic_set_safeout(path);
	if (ret) {
...
#endif
static int max77803_muic_set_path(struct max77803_muic_info *info, int path)
{
//	int ret;
...
#endif
	if ((info->max77803->pmic_rev == MAX77803_REV_PASS1) &&
	...
	max77803_update_reg(info->muic,
				MAX77803_MUIC_REG_CTRL1, ctrl1_val, ctrl1_msk);

	ctrl2_val = CTRL2_CPEn1_LOWPWD0;
	ctrl2_msk = CTRL2_CPEn_MASK | CTRL2_LOWPWD_MASK;

	max77803_update_reg(info->muic,
			MAX77803_MUIC_REG_CTRL2, ctrl2_val, ctrl2_msk);

	return 0;
}


#define MAX77803_PATH_FIX_USB_MASK \
	(BIT(EXTCON_USB_HOST) | BIT(EXTCON_SMARTDOCK) | BIT(EXTCON_AUDIODOCK))
#define MAX77803_PATH_USB_MASK \
	(BIT(EXTCON_USB) | BIT(EXTCON_JIG_USBOFF) | BIT(EXTCON_JIG_USBON))
#define MAX77803_PATH_AUDIO_MASK \
	(BIT(EXTCON_DESKDOCK) | BIT(EXTCON_CARDOCK))
#define MAX77803_PATH_UART_MASK	\
	(BIT(EXTCON_JIG_UARTOFF) | BIT(EXTCON_JIG_UARTON))

static int set_muic_path(struct max77803_muic_info *info)
{
	...
	dev_info(info->dev, "%s: current state=0x%x, path=%s\n",
			__func__, state, max77803_path_name[info->path]);

	if (state & MAX77803_PATH_FIX_USB_MASK)
		path = PATH_USB_AP;
	else if (state & MAX77803_PATH_USB_MASK) {
		/* set path to usb following usb_sel */
		if (info->muic_data->usb_sel == PATH_USB_CP)
			path = PATH_USB_CP;
		else
			path = PATH_USB_AP;
	} else if (state & MAX77803_PATH_UART_MASK) {
		/* set path to uart following uart_sel */
		if (info->muic_data->uart_sel == PATH_UART_CP)
			path = PATH_UART_CP;
		else
			path = PATH_UART_AP;
	} else if (state & MAX77803_PATH_AUDIO_MASK)
		path = PATH_AUDIO;
	else {
	...
	ret = max77803_muic_set_path(info, path);
	if (ret < 0) {
		dev_err(info->dev, "fail to set path(%s->%s),stat=%x,ret=%d\n",
				max77803_path_name[info->path],
				max77803_path_name[path], state, ret);
		return ret;
	}

	dev_info(info->dev, "%s: path: %s -> %s\n", __func__,
			max77803_path_name[info->path],
			max77803_path_name[path]);
	info->path = path;

	return 0;
}

static void _detected(struct max77803_muic_info *info, u32 new_state)
{
	u32 current_state;
	u32 changed_state;
	int index;
...
static int max77803_muic_handle_detach(struct max77803_muic_info *info)
{
	u8 ctrl2_val;

	/* Workaround: irq doesn't occur after detaching mHL cable */
	max77803_write_reg(info->muic, MAX77803_MUIC_REG_CTRL1,
					MAX77803_MUIC_CTRL1_BIN_0_000);

	/* Enable Factory Accessory Detection State Machine */

	max77803_update_reg(info->muic,
				MAX77803_MUIC_REG_CTRL2, CTRL2_CPEn0_LOWPWD1,
				CTRL2_CPEn_MASK | CTRL2_LOWPWD_MASK);

	max77803_read_reg(info->muic, MAX77803_MUIC_REG_CTRL2,
				&ctrl2_val);
	dev_info(info->dev, "%s: CNTL2(0x%02x)\n", __func__, ctrl2_val);
					...
					u8 status1, u8 status2)
{
	...
	dev_info(info->dev, "adc:%x adcerr:%x adc1k:%x  chgtyp:%x vb:%x dxovp:%x cable_type:%d\n",
		adc, adcerr, adc1k, chgtyp, vbvolt, dxovp, info->cable_type);
...
static void max77803_muic_detect_dev(struct max77803_muic_info *info, int irq)
{
	...
	if (intr == INT_ATTACH) {
		...
		max77803_muic_handle_attach(info, status[0], status[1]);
	} else if (intr == INT_DETACH) {
		dev_info(info->dev, "%s: DETACHED\n", __func__);
		max77803_muic_handle_detach(info);
	} else {
...
static int max77803_muic_irq_init(struct max77803_muic_info *info)
{
	...
	for (count = 1; count < 9 ; count ++) {
		max77803_read_reg(info->muic, count, &val);
...
static int max77803_muic_probe(struct platform_device *pdev)
{
...
static int max77803_muic_remove(struct platform_device *pdev)
{
